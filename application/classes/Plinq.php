...
class Plinq extends RecursiveArrayIterator
{
     ...
     *
     * @param mixed $closure     a closure that returns boolean
     ...
     * @return array
     */
    public function SelectKV($closure) {
        $applicables = $this->GetApplicables($closure, 0, self::PLINQ_CLOSURE_RETURN_TYPE_OBJECT);

        $return_array = array();

        foreach($applicables as $item) {
            $return_array[$item[0]] = $item[1];
        }
     ...
     *
     * @param callable $closure    a closure that returns any numeric type (int, float etc.)
    ...
    public function Average($closure)
    {
        ...
        for($i = 0; $i < $total; $i++)
        {
            ...
            if(!is_numeric(($result = $closure($key, $value))))
                continue;
     ...
     *
     * @param mixed $closure    a closure that selects the order key, key can be anything
     ...
     *
     * @param mixed $closure    a closure that returns an item to append, item can be any type.
     * @return Plinq
     */
     ...
     *
     * @param mixed $closure    a closure that returns an item that is a form of Array.
     * @return Plinq
     */
     ...
     *
     * @param callable $closure    a closure that returns an item as key, item can be any type.
     * @return
     */
        ...
        for($i = 0; $i < $total; $i++)
        {
            ...
            switch($closureReturnType)
            {
                case self::PLINQ_CLOSURE_RETURN_TYPE_BOOL:
                    if(!is_bool(($returned = $closure($storedKey, $stored))) || !$returned)
                        continue;
                ...
                case self::PLINQ_CLOSURE_RETURN_TYPE_OBJECT:
                    $applicables[$storedKey] = $closure($storedKey, $stored);
                    $totalApplicable++;
                    break;
            }
    ...
    public function DistinctEx($closure, $strict = true) {
        $keys = array();
        $output = array();

        foreach($this as $storedKey => $storedValue) {
            $generatedKey = $closure($storedKey, $storedValue);

            $skipItem = false;

            foreach($keys as $someKey) {
                if($strict && $someKey === $generatedKey) {
                    $skipItem = true;
                    break;
                }
